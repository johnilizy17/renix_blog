...
import { getBlogPost, getBlogPostsearch } from "../services/authBlog";
...
export default function Widgets({ newsResults, randomUsersResults }) {
  ...
  async function fetchBlog(e) {
    try {
      const data = await getBlogPostsearch(e.target.value)
    ...
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
      ...
      <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
        ...
        <AnimatePresence>
          ...
          {display ?
            posts.slice(0, articleNum).map((article) => (
              <motion.div
                key={article.title}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <Search key={article._id} article={article} />
              </motion.div>
            ))
            :
